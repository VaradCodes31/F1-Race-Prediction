...
# Load the 2024 Spanish GP session data (previous race for Barcelona)
...
# Clean air race pace from Spanish GP 2024 (using your racepace.py on Spanish GP data)
clean_air_race_pace = {
    "VER": 76.902, "HAM": 76.379, "LEC": 74.796, "NOR": 75.202, "ALO": 77.612,
    "PIA": 75.122, "RUS": 78.463, "SAI": 79.037, "STR": 78.982, "HUL": 78.582,
    "OCO": 77.204, "GAS": 77.923, "ALB": 77.007
}

# Updated qualifying data for Spanish GP 2025
...
# Get weather data for Barcelona, Spain
...
forecast_time = "2025-06-15 13:00:00"  # Spanish GP race time
...
# Average position change at Spanish GP (qualifying pos - finish pos); positive means losing positions
average_position_change_spanish = {
    "VER": -0.8,  # Usually gains positions
    "NOR": 0.2,   # Steady
    "PIA": 0.5,   # Slight position loss
    "RUS": 0.3,   # Steady
    "SAI": -0.4,  # Home advantage, usually gains
    "ALB": 0.6,   # Usually loses positions
    "LEC": -0.2,  # Slight gains
    "OCO": 0.1,   # Steady
    "HAM": -0.1,  # Slight gains
    "STR": 0.8,   # Usually loses positions
    "GAS": 0.0,   # Neutral
    "ALO": -0.3,  # Usually gains (home advantage)
    "HUL": 0.4    # Slight position loss
}
qualifying_2025["AveragePositionChange"] = qualifying_2025["Driver"].map(average_position_change_spanish)
...
plt.title("Effect of Clean Air Race Pace on Predicted Spanish GP Results")
plt.tight_layout()
plt.show()
